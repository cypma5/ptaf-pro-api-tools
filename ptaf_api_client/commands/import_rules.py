...
class ImportRulesCommand(BaseCommand):
    ...
    def get_available_actions(self) -> Optional[List[dict]]:
        """Получает список доступных действий"""
        if not self.ensure_auth():
            return None
        
        url = f"{self.config.api_path}/config/actions"
        response = self.http_client.get(url)
        
        if response and response.status_code == 200:
            actions = response.json()
            return actions.get('items', []) if isinstance(actions, dict) else actions
        
        return None
    
    def create_rule(self, template_id: str, rule_data: dict):
        """Создает новое правило"""
        url = f"{self.config.api_path}/config/policies/templates/with_user_rules/{template_id}/rules"
        return self.http_client.post(url, json=rule_data)
    
    def update_rule(self, template_id: str, rule_id: str, update_data: dict):
        ...
        return self.http_client.patch(url, json=update_data)
    
    def enable_rule(self, template_id: str, rule_id: str, enable: bool = True):
        """Включает или отключает правило"""
        url = f"{self.config.api_path}/config/policies/templates/with_user_rules/{template_id}/rules/{rule_id}"
        return self.http_client.patch(url, json={"enabled": enable})
    
    def import_single_rule(self, template_id: str, file_path: str, selected_action_ids: Optional[List[str]] = None, enable_after_import: bool = False) -> bool:
        ...
        try:
            ...
            if rule_name in existing_rules_dict:
                ...
                if response is None:
                    error_msg = "Не удалось выполнить запрос на обновление"
                    print(error_msg)
                    self.failed_files.append({
                        'file': file_path,
                        'rule': rule_name,
            ...
            print(f"Ошибка при обработке файла {file_path}: {error_msg}")
            self.failed_files.append({
                'file': file_path,
    ...
    def execute(self, source_dir: str, tenant: Optional[Tenant] = None):
        """Основной метод импорта правил"""
        if tenant:
        ...
        json_files = [f for f in os.listdir(source_dir) if f.endswith('.ptafpro')]
        ...
        for filename in json_files:
            file_path = os.path.abspath(os.path.join(source_dir, filename))
